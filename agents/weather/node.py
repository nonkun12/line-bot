...
_PREFECTURE_FALLBACKS: dict[str, tuple[str, float, float]] = {
    "北海道": ("北海道", 43.0646, 141.3468), "青森県": ("青森県", 40.8244, 140.7400),
    "岩手県": ("岩手県", 39.7036, 141.1527), "宮城県": ("宮城県", 38.2682, 140.8694),
    "秋田県": ("秋田県", 39.7186, 140.1024), "山形県": ("山形県", 38.2404, 140.3633),
    "福島県": ("福島県", 37.7503, 140.4676), "茨城県": ("茨城県", 36.3418, 140.4468),
    "栃木県": ("栃木県", 36.5658, 139.8836), "群馬県": ("群馬県", 36.3911, 139.0608),
    "埼玉県": ("埼玉県", 35.8569, 139.6489), "千葉県": ("千葉県", 35.6051, 140.1233),
    "東京都": ("東京都", 35.6762, 139.6503), "神奈川県": ("神奈川県", 35.4478, 139.6425),
    "新潟県": ("新潟県", 37.9026, 139.0236), "富山県": ("富山県", 36.6953, 137.2113),
    "石川県": ("石川県", 36.5947, 136.6256), "福井県": ("福井県", 36.0652, 136.2216),
    "山梨県": ("山梨県", 35.6639, 138.5683), "長野県": ("長野県", 36.6513, 138.1810),
    "岐阜県": ("岐阜県", 35.3912, 136.7223), "静岡県": ("静岡県", 34.9756, 138.3828),
    "愛知県": ("愛知県", 35.1802, 136.9066), "三重県": ("三重県", 34.7303, 136.5086),
    "滋賀県": ("滋賀県", 35.0045, 135.8686), "京都府": ("京都府", 35.0210, 135.7556),
    "大阪府": ("大阪府", 34.6863, 135.5197), "兵庫県": ("兵庫県", 34.6913, 135.1830),
    "奈良県": ("奈良県", 34.6851, 135.8049), "和歌山県": ("和歌山県", 34.2260, 135.1675),
    "鳥取県": ("鳥取県", 35.5039, 134.2383), "島根県": ("島根県", 35.4723, 133.0505),
    "岡山県": ("岡山県", 34.6618, 133.9344), "広島県": ("広島県", 34.3963, 132.4596),
    "山口県": ("山口県", 34.1861, 131.4705), "徳島県": ("徳島県", 34.0658, 134.5593),
    "香川県": ("香川県", 34.3401, 134.0434), "愛媛県": ("愛媛県", 33.8416, 132.7657),
    "高知県": ("高知県", 33.5597, 133.5311), "福岡県": ("福岡県", 33.6064, 130.4183),
    "佐賀県": ("佐賀県", 33.2494, 130.2988), "長崎県": ("長崎県", 32.7448, 129.8737),
    "熊本県": ("熊本県", 32.7898, 130.7417), "大分県": ("大分県", 33.2382, 131.6126),
    "宮崎県": ("宮崎県", 31.9111, 131.4239), "鹿児島県": ("鹿児島県", 31.5602, 130.5581),
...
def _weather_code_text(code: int) -> str:
    codes = {
        ...
        51: "弱い霧雨", 53: "霧雨", 55: "強い霧雨", 61: "弱い雨", 63: "雨",
        65: "強い雨", 71: "弱い雪", 73: "雪", 75: "強い雪", 80: "にわか雨",
        81: "にわか雨", 82: "強いにわか雨", 95: "雷雨", 96: "雷雨", 99: "強い雷雨",
    }
...
def _wttr_condition_text(value: str) -> str:
    ...
    mapping = {
        "clear": "快晴", "sunny": "晴れ", "partly cloudy": "晴れ時々曇り", "cloudy": "曇り",
        "overcast": "曇り", "mist": "霧", "fog": "霧", "light drizzle": "弱い霧雨",
        "patchy light drizzle": "弱い霧雨", "light rain": "弱い雨", "patchy light rain": "弱い雨",
        "moderate rain": "雨", "heavy rain": "強い雨", "light sleet": "みぞれ", "light snow": "弱い雪",
        "moderate snow": "雪", "heavy snow": "強い雪", "thundery outbreaks possible": "雷雨の可能性",
        "light rain shower": "にわか雨", "moderate or heavy rain shower": "強いにわか雨",
...
def _normalize_location(location: str) -> str:
    ...
    return value[:-1] if value.endswith("の") else value
        ...
        for attempt in range(2):
            try:
                response = requests.get(
                    ...
                    params={"name": query, "count": 10, "language": "ja", "format": "json", "countryCode": "JP"},
                    timeout=10,
                )
...
def _geocode_location(location: str) -> tuple[str, float, float]:
    ...
    known_city = _CITY_FALLBACKS.get(normalized)
    ...
    return str(result.get("name") or location), float(result["latitude"]), float(result["longitude"])


def _get_weather_from_wttr(latitude: float, longitude: float) -> dict:
    response = requests.get(
        f"https://wttr.in/{latitude:.4f},{longitude:.4f}", params={"format": "j1", "lang": "ja"},
        headers={"User-Agent": "LINE-AI-Secretary/1.0"}, timeout=15,
    )
    response.raise_for_status()
    ...
    return {"current": {"temperature_2m": float(current["temp_C"]), "temperature_2m_unit": "°C", "weather_code_text": _wttr_condition_text(description)}, "provider": "wttr.in"}
    ...
    for attempt in range(2):
        try:
            response = requests.get(
                ...
                params={"latitude": latitude, "longitude": longitude, "current": "temperature_2m,weather_code", "timezone": "Asia/Tokyo"},
                timeout=20,
            )
...
def weather_agent_node(state):
    try:
        location = _extract_weather_location(str(state.get("raw_message", "")))
        if not location:
        ...
        if data.get("provider") == "wttr.in":
            weather_text = str(current.get("weather_code_text") or "天気不明")
        else:
            weather_text = _weather_code_text(int(current["weather_code"]))
        result = {"text": f"{resolved_name}の現在の天気です。\n天気: {weather_text}\n気温: {temperature}{unit}", "success": True, "provider": str(data.get("provider") or "open-meteo"), "location": resolved_name}
    except Exception as exc:
        result = {"text": "天気情報を取得できませんでした。", "success": False, "provider": "open-meteo", "error": str(exc)}
