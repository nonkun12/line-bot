...
_DELETE_ALL_MEMORY_PATTERN = re.compile(r"(記憶|memory|メモ).*(全部|すべて|全て|リセット|削除|消去)")
...
def handle_save_memory(message, user_id, call_mcp_tool):
    if not any(x in message for x in ["覚えて", "覚えといて", "覚えてる？", "覚えている？"]):
        return None
    if (("明日" in message and "時" in message) or "教えて" in message or
        "覚えていること" in message or "覚えてること" in message or
        "何を覚えてる" in message or "何を覚えている" in message or
        "覚えてる？" in message or "覚えている？" in message):
        return None
    text = re.sub(r"^覚えて\s*[:：]?\s*", "", message) if message.startswith("覚えて") else message.strip()
    key, value = "memory", text
    m = re.search(r"(?:私の)?名前は(.+?)(?:です|、|。|$)", message)
    if m:
        key, value = "name", m.group(1).strip()
    return call_mcp_tool("save_memory", {"user_id": user_id, "key": key, "value": value})


def handle_delete_memory(message, user_id, call_mcp_tool):
    if not any(keyword in message for keyword in ["忘れて", "削除して", "削除する", "消して", "消す"]):
        return None
    key = _find_memory_target(message, user_id, call_mcp_tool)
    result = call_mcp_tool("delete_memory", {"user_id": user_id, "key": key})
    with _pending_confirm_lock:
    ...
    if name:
        try:
            ...
            if isinstance(name_data, dict):
                ...
                item = next((x for x in name_data if isinstance(x, dict) and x.get("key") == "name"), None)
                if item:
                    name = item.get("value", name)
        except Exception:
            pass
    return f"あなたの名前は {name} です。" if name not in invalid_values else "名前はまだ記憶されていません。"


def handle_get_all_memory(message, user_id, call_mcp_tool):
    query_keywords = ["何を覚えて", "何を覚えてる", "何を覚えている", "覚えてる？", "覚えている？", "覚えてるの？", "覚えているの？"]
    ...
    try:
        ...
        lines = [f"- {item.get('key', '')}: {item.get('value', '')}" for item in data if item.get('value', '')]
        return "覚えている情報は以下です。\n" + "\n".join(lines)
...
def handle_delete_all_memory(message, user_id, call_mcp_tool):
    if (message in ["記憶全部削除", "記憶をすべて削除", "記憶を全部削除", "全ての記憶を削除", "全部の記憶を削除", "memoryを全部削除して", "保存しているメモを全部削除して", "保存している記憶を消して", "覚えていること全部消して", "覚えていることを全部削除して", "記憶をリセットして", "memoryをリセットして"] or _DELETE_ALL_MEMORY_PATTERN.search(message)):
        with _pending_confirm_lock:
...
def handle_memory_message(message, user_id, call_mcp_tool):
    for handler in [handle_delete_all_memory, handle_save_memory, handle_delete_memory, handle_get_name, handle_get_all_memory]:
