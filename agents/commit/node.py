...
def commit_node(state):
    ...
    if staged_before.returncode != 0:
        return _result(state, {"committed": False, "error": staged_before.stderr, "paths": paths})
    ...
    if add.returncode != 0:
        return _result(state, {"committed": False, "error": add.stderr, "paths": paths})

    staged_after = _run_git(["diff", "--cached", "--name-only", "--"], cwd=workdir)
    if staged_after.returncode != 0:
        return _result(state, {"committed": False, "error": staged_after.stderr, "paths": paths})
    ...
    if commit.returncode != 0:
        return _result(state, {"committed": False, "error": commit.stderr, "paths": paths})
