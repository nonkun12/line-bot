...
class QualityRuntime:
    """Run quality gates without conflating testing, debugging, and refactoring."""
    ...
    def _execute(self, task: AgentTask) -> RuntimeTaskResult:
        ...
        if executor is None:
            return RuntimeTaskResult(task, AgentResult(task.task_id, False, f"missing executor for role: {task.role.value}"))
        try:
    ...
    def _quality_repair(self, failed: RuntimeTaskResult, attempt: int, completed: list[RuntimeTaskResult]) -> RuntimeTaskResult | None:
    ...
    def run(self, tasks: Sequence[AgentTask]) -> RuntimeReport:
        ...
        if manager is not None:
            ...
            if not result.result.success:
                return RuntimeReport(tuple(completed), manager.task_id, result.result.summary, rounds, attempts)

        implementer = self._execute(templates[AgentRole.IMPLEMENTER])
        completed.append(implementer)
        if not implementer.result.success:
            return RuntimeReport(tuple(completed), implementer.task.task_id, implementer.result.summary, rounds, attempts)

        tester = self._execute(templates[AgentRole.TESTER])
        completed.append(tester)
        while not tester.result.success:
            if attempts >= self._max_rounds - 1:
                return RuntimeReport(tuple(completed), tester.task.task_id, tester.result.summary, rounds, attempts)
            attempts += 1
            rounds = max(rounds, attempts + 1)
            repaired = self._quality_repair(tester, attempts, completed)
            if repaired is None:
                return RuntimeReport(tuple(completed), tester.task.task_id, "TEST failed; Debugger and Refactorer are required", rounds, attempts)
            if repaired.task.role is AgentRole.DEBUGGER or repaired.task.role is AgentRole.REFACTORER:
                return RuntimeReport(tuple(completed), repaired.task.task_id, repaired.result.summary, rounds, attempts)
            tester = repaired
        ...
        while not reviewer.result.success:
            if attempts >= self._max_rounds - 1:
                return RuntimeReport(tuple(completed), reviewer.task.task_id, reviewer.result.summary, rounds, attempts)
            attempts += 1
            rounds = max(rounds, attempts + 1)
            repaired = self._quality_repair(reviewer, attempts, completed)
            if repaired is None:
                return RuntimeReport(tuple(completed), reviewer.task.task_id, "Review failed; Debugger and Refactorer are required", rounds, attempts)
        ...
        if not integrator.result.success:
            return RuntimeReport(tuple(completed), integrator.task.task_id, integrator.result.summary, rounds, attempts)
        return RuntimeReport(tuple(completed), rounds=rounds, repair_attempts=attempts, integration_ready=True)
