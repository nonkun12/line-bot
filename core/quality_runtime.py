    ...
    def _execute(self, task: AgentTask) -> RuntimeTaskResult:
        ...
        if result.success and self._execution_safety_gate is not None and task.role is not AgentRole.MANAGER:
            try:
                ...
                return RuntimeTaskResult(
                    task,
                    AgentResult(task.task_id, False, f"execution safety gate error: {type(exc).__name__}: {exc}"),
                )
            if not verified:
                return RuntimeTaskResult(
                    task,
                    AgentResult(task.task_id, False, "execution safety gate rejected actual worktree state"),
                )
