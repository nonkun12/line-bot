...
class MultiAgentRuntime:
    ...
    ``max_workers`` is deliberately restricted to one until executors have
    isolated git worktrees. The development loop requires an explicit
    Implementer, Tester, Reviewer, and Integrator task. Reviewer/Tester failure
    can create one bounded Repairer cycle followed by retest and re-review.
    """
        ...
        for batch in batches:
            ...
            for task, result in results:
                if not result.success:
                    return RuntimeReport(
                        tuple(completed), task.task_id, result.summary or "agent task failed"
                    )
    ...
    def run_development(
        ...
        *,
        ...
        """Run the guarded Implementer/Tester/Reviewer/Integrator loop.

        Integration is only represented as ready after an actual successful
        Integrator task. This layer never merges or pushes anything.
        """
        required_roles = {
        ...
        integrator_template = next(
            task for task in tasks if task.role is AgentRole.INTEGRATOR
        )
        completed: list[RuntimeTaskResult] = []
        current = tuple(tasks)
        repair_attempts = 0
        ...
        for round_number in range(1, self._max_rounds + 1):
            ...
            if not report.success:
                ...
                if failed is None:
                    return RuntimeReport(
                        tuple(completed), report.failed_task_id, report.error,
                ...
                if failed.role not in (AgentRole.TESTER, AgentRole.REVIEWER):
                    return RuntimeReport(
                        tuple(completed), failed.task_id, report.error,
                        round_number, repair_attempts,
                    )
                if repair_planner is None or repair_attempts >= self._max_rounds - 1:
                    return RuntimeReport(
                        tuple(completed), failed.task_id,
                        report.error or "gate failed", round_number, repair_attempts,
                    )
                repair_attempts += 1
                try:
                    repair_task = repair_planner.repair_task(
                        failed, failed_result, repair_attempts
                    )
                except Exception as exc:
                    return RuntimeReport(
                        tuple(completed), failed.task_id,
                ...
                if repair_task.role is not AgentRole.REPAIRER:
                    return RuntimeReport(
                        tuple(completed), repair_task.task_id,
                ...
                if not repair_report.success:
                    return RuntimeReport(
                        tuple(completed), repair_report.failed_task_id,
                        repair_report.error, round_number, repair_attempts,
                    )
                current = self._follow_up_tasks(
                    failed, repair_task, integrator_template, round_number
                )
                continue
            ...
            if reviewer is None:
                return RuntimeReport(
                    tuple(completed), error="reviewer gate missing",
                    rounds=round_number, repair_attempts=repair_attempts,
                )
            if integrator is None:
                return RuntimeReport(
                    tuple(completed), error="integrator gate missing",
                    rounds=round_number, repair_attempts=repair_attempts,
                )
            return RuntimeReport(
                tuple(completed), rounds=round_number,
                repair_attempts=repair_attempts, integration_ready=True,
            )

        return RuntimeReport(
            tuple(completed), error="development round limit reached",
    ...
    def _follow_up_tasks(
        failed: AgentTask,
        repair: AgentTask,
        integrator: AgentTask,
        round_number: int,
    ) -> tuple[AgentTask, ...]:
        suffix = f":round{round_number}"
        if failed.role is AgentRole.TESTER:
            retest_id = f"{failed.task_id}:retest{suffix}"
            review_id = f"{failed.task_id}:review{suffix}"
            gate_id = f"{integrator.task_id}:gate{suffix}"
            return (
                AgentTask(
                    task_id=retest_id,
                ...
                AgentTask(
                    ...
                    resources=failed.resources,
                    depends_on=(retest_id,),
                ),
                AgentTask(
                    task_id=gate_id,
                    role=AgentRole.INTEGRATOR,
                    instruction=f"Run the integration gate after {review_id}.",
                    resources=integrator.resources,
                    depends_on=(review_id,),
                ),
            )
        if failed.role is AgentRole.REVIEWER:
            retest_id = f"{failed.task_id}:retest{suffix}"
            review_id = f"{failed.task_id}:rereview{suffix}"
            gate_id = f"{integrator.task_id}:gate{suffix}"
            return (
                AgentTask(
                    task_id=retest_id,
    ...
    def _latest_role_result(
        completed: Sequence[RuntimeTaskResult], role: AgentRole
