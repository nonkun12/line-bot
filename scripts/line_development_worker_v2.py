...
def choose_file(client: Groq, instruction: str, files: list[str]) -> str | None:
    system = """Choose exactly one repository file for the requested change. Return JSON only: {\"file\":\"path\"} or {\"file\":null}. Never choose security, credential, deployment, workflow, or worker files."""
    listing = "\n".join(files)
...
def apply_plan(plan: dict) -> tuple[bool, str, list[str]]:
    ...
    for change in plan["changes"]:
        ...
        old = change["old"]
        if text.count(old) != 1:
            return False, f"anchor_count_{path}:{text.count(old)}", touched
...
def build_plan(client: Groq, instruction: str, chosen: str, context: str, test_output: str | None = None) -> dict:
    system = """You are editing ONE repository file. Return JSON only.
Format for a real change: {\"no_change\":false,\"changes\":[{\"file\":\"exact path\",\"old\":\"exact existing text\",\"new\":\"replacement text\"}]}
Format when no change is needed or the request cannot be safely implemented from the supplied context: {\"no_change\":true}.
Rules: one file only; old must be an exact substring from the supplied context; keep the change minimal; never modify credentials, security, deployment, workflow, or worker logic.
"""
    prompt = f"Instruction:\n{instruction}\n\nSelected file:\n{chosen}\n\nCurrent file context (may be truncated; do not invent unseen text):\n{context}"
    if test_output:
        prompt += f"\n\nPytest failure to repair:\n{test_output[-4000:]}"
...
def main() -> int:
    ...
    context = context_for(chosen)
    plan = build_plan(client, instruction, chosen, context)
    ...
    while not passed and attempts <= MAX_REPAIR_ATTEMPTS:
        ...
        attempts += 1
        try:
            repair = build_plan(client, instruction, chosen, context, output)
