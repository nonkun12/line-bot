...
def record_wav(path: Path, *, seconds: float, sample_rate: int = 16_000) -> None:
    if sd is None:
        raise RuntimeError("sounddevice is required; install it with: python -m pip install sounddevice") from _SOUNDDEVICE_ERROR
...
def request_speaker(base_url: str, internal_key: str, user_id: str, audio_path: Path) -> str:
    headers = {"X-Internal-Key": internal_key}
    with httpx.Client(timeout=60) as client:
        with audio_path.open("rb") as audio:
            transcription = client.post(f"{base_url.rstrip('/')}/api/voice/transcribe", headers=headers, files={"file": (audio_path.name, audio, "audio/wav")})
        ...
        response = client.post(f"{base_url.rstrip('/')}/api/voice", headers=headers, json={"user_id": user_id, "message": text})
