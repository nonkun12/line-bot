...
def fetch_ai_secretary_facts(user_id):
    ...
    try:
        facts["memories"] = _parse_mcp_json_list(
            call_mcp_tool("get_all_memory", {"user_id": user_id})
        )
    except Exception as exc:
        print("AI REPORT: GET_ALL_MEMORY ERROR:", exc)
    try:
        facts["reminders"] = _parse_mcp_json_list(
            call_mcp_tool("list_reminders", {"user_id": user_id})
        )
    except Exception as exc:
