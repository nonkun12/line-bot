...
def route_from_review(state: AgentState) -> str:
    ...
    if status == "pending":
        return "review_agent"
    if status == "failed" and review_result.get("ai_review") and review_result.get("retryable"):
        return "fix_agent"
    return "finalizer"


def route_from_merge(state: AgentState) -> str:
    merge_result = state.get("merge_result") or {}
    return "deploy_agent" if merge_result.get("merged") else "finalizer"
...
def build_graph(*, checkpointer=None, interrupt_after=None, interrupt_before=None):
    ...
    builder.add_conditional_edges("review_agent", route_from_review, {"review_agent": "review_agent", "merge_agent": "merge_agent", "fix_agent": "fix_agent", "finalizer": "finalizer"})
