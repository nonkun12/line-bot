...
def test_append_development_result_uses_single_audit_row():
    ...
    writer.append_development_result(instruction="run autonomous development", status="PASS", target_path="tests/test_example.py", branch="line-dev/123", detail="development branch pushed", base_sha="base", produced_sha="produced")
    assert len(calls) == 1
    ...
    assert calls[0]["body"]["values"][0][1:] == ["PASS", "run autonomous development", "tests/test_example.py", "line-dev/123", "base", "produced", "development branch pushed", "PASS"]
