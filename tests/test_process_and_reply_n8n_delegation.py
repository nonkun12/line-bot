"""app._process_and_reply() のn8n委譲分岐とGateway経路のテスト。"""
import pytest
import app
...
def test_process_and_reply_delegates_to_n8n_when_webhook_url_set(monkeypatch, capture_messaging_api):
    ...
    monkeypatch.setattr(
        ...
        lambda *args: (_ for _ in ()).throw(
            AssertionError("generate_reply should not be called when delegating to n8n")
        ),
    )
...
def test_process_and_reply_uses_local_path_when_webhook_url_unset(monkeypatch, capture_messaging_api):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("_delegate_to_n8n should not be called when N8N_WEBHOOK_URL is unset")
        ),
    )
...
def test_process_and_reply_local_path_routes_through_shared_ai_gateway(monkeypatch, capture_messaging_api):
    monkeypatch.setattr(app, "N8N_WEBHOOK_URL", "")
    captured = {}

    def fake_handle(request):
        captured["request"] = request
        return AIResponse(text="Gateway経由の返信")

    monkeypatch.setattr(app.app.ai_gateway, "handle", fake_handle)
    app._process_and_reply(_FakeEvent(), "user-123", "こんにちは")

    assert captured["request"].user_id == "user-123"
    assert captured["request"].message == "こんにちは"
    assert captured["request"].channel == "line"
    assert captured["request"].metadata["route"] == "line_callback"
    assert len(capture_messaging_api) == 1
    kind, request = capture_messaging_api[0]
    assert kind == "reply"
    assert request.messages[0].text == "Gateway経由の返信"
