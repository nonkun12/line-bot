...
def test_test_and_review_have_independent_repair_budgets():
    executor = FakeExecutor([
        True, True, False,
        True, True, True,
        False,
...
def test_max_rounds_one_disables_repair_retries_explicitly():
    executor = FakeExecutor([True, True, False])
    runtime = QualityRuntime({role: executor for role in AgentRole}, max_rounds=1)

    report = runtime.run(base_tasks())

    assert not report.success
    assert report.failed_task_id == "tester"
    assert [call.role for call in executor.calls] == [
        AgentRole.MANAGER,
        AgentRole.IMPLEMENTER,
        AgentRole.TESTER,
